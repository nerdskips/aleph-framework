...
class ZAPISender:
    ...
    async def send_response(self, phone: str, text: str) -> None:
        """Send agent response with humanized delivery.

        Splits by paragraph, sends each as separate message with
        random delay between them. Appends disclaimer if enabled.
        """
        messaging = self.config.messaging

        if self.config.debug.dry_run:
            logger.info("[DRY RUN] Would send to %s: %s", phone, text[:100])
            return

        # Split into paragraphs
        if messaging.send_as_paragraphs:
        ...
        for i, part in enumerate(parts):
            ...
            if i < len(parts) - 1:
                delay_ms = random.randint(
                    messaging.delay_min_ms,
                    messaging.delay_max_ms,
                )
                await asyncio.sleep(delay_ms / 1000.0)

        logger.info(
            "Sent %d message(s) to %s (%d chars total)",
            len(parts), phone, len(text),
        )

    async def send_notification(self, phone: str, text: str) -> str | None:
        """Send a notification message (for escalation, etc).
        Returns the messageId of the sent message (for quote tracking)."""
        if self.config.debug.dry_run:
            logger.info("[DRY RUN] Notification to %s: %s", phone, text[:100])
            return None

        result = await self._send_text(phone, text)
        if result:
    ...
    async def _send_text(self, phone: str, text: str) -> dict | None:
        ...
        try:
            ...
            data = response.json()
            logger.debug("Z-API sent to %s: %s", phone, text[:50])
            return data
        except httpx.HTTPError as e:
            logger.error("Z-API send failed to %s: %s", phone, str(e)[:200])
            return None
